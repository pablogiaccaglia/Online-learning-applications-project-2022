...
from Part3.GPTS_Learner import GPTS_Learner
from Part6.SwUCB1Learner import SwUCB1Learner
...
for day in range(days):
    ...
    sw_ucb1.append(sim_obj_2["profit_campaign"][-1] - np.sum(super_arm))
    # solve comb problem for tomorrow
    super_arm = comb_learner.pull_super_arm()
    # -----------------------------------------------------------------
    if day % 20 == 0:
...
print(f"GTS profit:\t {sum(sw_ucb1):.4f}€")
plt.close()
...
axs[0].plot(d, sw_ucb1)

axs[1].set_xlabel("days")
axs[1].set_ylabel("cumulative reward")
axs[1].plot(d, np.cumsum(rewards_knapsack_agg))
axs[1].plot(d, np.cumsum(sw_ucb1))

axs[2].set_xlabel("days")
axs[2].set_ylabel("CUCB1 cumulative regret")
axs[2].plot(d, np.cumsum(np.array(rewards_knapsack_agg) - np.array(sw_ucb1)))

axs[3].set_xlabel("days")
axs[3].set_ylabel("CUCB1 regret")
axs[3].plot(d, np.array(rewards_knapsack_agg) - np.array(sw_ucb1))
plt.show()
